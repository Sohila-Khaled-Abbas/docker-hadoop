...
docker compose exec hadoop hdfs dfs -mkdir -p /example/python/input
docker compose exec hadoop hdfs dfs -rm -r -f /example/python/output
...
docker compose exec hadoop chmod +x /tmp/mapper.py /tmp/reducer.py
docker compose exec hadoop hdfs dfs -put -f /tmp/sample.txt /example/python/input/

echo "=== 3. Executing Hadoop Streaming MapReduce Job ==="
docker compose exec hadoop hadoop jar "$STREAMING_JAR" \
...
docker compose exec hadoop hdfs dfs -cat /example/python/output/part-00000
