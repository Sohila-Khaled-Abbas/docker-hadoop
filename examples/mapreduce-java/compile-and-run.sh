...
docker compose exec hadoop bash -c "
...
docker compose exec hadoop hdfs dfs -mkdir -p /example/java/input
docker compose exec hadoop hdfs dfs -rm -r -f /example/java/output
docker compose exec hadoop hdfs dfs -put -f /usr/local/hadoop/etc/hadoop/*.xml /example/java/input/

echo "=== 4. Running YARN MapReduce Job ==="
docker compose exec hadoop yarn jar /tmp/wordcount.jar WordCount /example/java/input /example/java/output

echo "=== 5. Reading Output from HDFS ==="
docker compose exec hadoop hdfs dfs -cat /example/java/output/part-r-00000 | head -n 30
